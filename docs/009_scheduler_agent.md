...
- **Agent preamble (fast path):** Heartbeat turns skip the extra Ollama call that extracts success criteria and **omit MCP `tools/list`** from the prompt (built-in tools/skills/agents still appear). That keeps beats short and avoids MCP stdio/SSE on the shared runtime during periodic checks. Full MCP remains available for Discord/UI/scheduler runs. Heartbeat also **skips Redmine create-context fetches** (the fixed checklist wording contains phrases like “write …” that would otherwise false-trigger the create/update heuristic) and **skips the completion-verification LLM call** at the end of the turn.

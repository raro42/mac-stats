...
| Version | What |
| --- | --- |
| **v0.1.898** | Instant: `user-info.json` size (`user info size` / `how big is user info` / `user details size`) |
...
- Next fuel: `credential_accounts.json` size / `.config.env` size / agent file sizes, or a design-review polish when screenshots work.

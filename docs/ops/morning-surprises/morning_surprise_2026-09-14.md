...
In AI Chat or Discord: `/keep-kurtosis` or `excess kurtosis keep gap` (beside `/keep-skew` skewness, `/keep-cv` CV, `/keep-mad` MAD, `/keep-std` sample std, `/keep-pace` average, `/keep-median` median, `/keep-range` min–max, `/keep-p90` p90, and `/keep-iqr` IQR).
...
- Install/kickstart after v0.1.1064; Discord Ready.

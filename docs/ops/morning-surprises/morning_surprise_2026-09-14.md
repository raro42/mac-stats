...
Open the CPU window: when Heat is Fair and Temp is under 70°C, the Temperature ring and TEMP sparkline show a soft amber wash (no pulse). Serious/Critical still pulse hot. Instant ratchet: `/keep-p95` or `95th percentile keep gap` (tail gap). Also `/keep-histogram` for minute buckets; `/keep-gini` for inequality (0 = even pace).

## Notes

- Digester open empty this tick; design review due=false (grace). Fuel = standing backlog `/keep-p95` after keep-histogram.
- Install/kickstart after v0.1.1070; confirm Discord Ready.

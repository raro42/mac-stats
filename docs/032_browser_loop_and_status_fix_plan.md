...
- ~~Review and optimize the `ollama.rs` tool loop for better performance and error handling.~~ **Done:** Unknown-tool handling: the catch-all no longer silently skips; unknown tools now produce a user-facing hint ("Unknown tool \"X\". Use one of the available tools...") and a `tracing::warn!` so the model gets feedback and logs are traceable. Error short-circuiting (browser/RUN_CMD) and browser cap/repetition logic were already in place; no further performance change in this pass.
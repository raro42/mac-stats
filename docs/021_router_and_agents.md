...
3. **How do agents and the API router work together?**
   ...
   - **`OllamaRequest::silent_user_output`** (OpenClaw-style silent-expected): When `true`, the orchestrator does not send `status_tx` lines, passes `None` into the tool loop for status/draft (no per-tool Discord draft progress, no `ATTACH:` forwarding via the active status channel), and skips several user-only reply appendices (agent transcript block, screenshot/browser-cap notes, model-catalog warning prefix). Discord returns `OllamaReply::silent_user_output` so the outbound path can delete the draft placeholder instead of flushing `(No reply text.)` when the final is empty. **Heartbeat** sets this flag. **Debug Discord builds** can set env `MAC_STATS_DEV_SILENT_DISCORD_OLLAMA=1` to exercise the path. The tool loop still sets `forward_substantive_output` after merging tool results so `BrowserSessionLost` / `ServiceUnavailable` retry does not treat silent turns with real tool work as an empty pipeline.
...
- **Browser**: After connection retry or CDP→HTTP failover; or on non-transient errors (e.g. invalid URL). **CDP health check failure** (`Browser unresponsive`): no second retry—the session is already cleared inside `check_browser_alive` / `clear_browser_session_on_error`, including messages that also mention “connection closed” (health path takes precedence over plain transport retry).

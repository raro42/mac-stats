...
- **Integration:** `cargo check` and `cargo clippy` pass (44 warnings; no errors). `diff src/ollama.js src-tauri/dist/ollama.js` empty (in sync). `toggle_cpu_window`, `set_chat_verbosity` in `tauri::generate_handler![]` in `lib.rs` (L228, L237); `run_due_monitor_checks()` in `lib.rs` (L372, background thread, 30s).
- **Smoke:** `cargo build --release` succeeded (v0.1.45). `pkill -f mac_stats` then `nohup ./target/release/mac_stats --cpu -vv` started in background; `pgrep -fl mac_stats` confirmed process (pid 83834). `~/.mac-stats/debug.log`: Discord gateway, Task review and Scheduler (2 entries) threads spawned, 8 agents loaded with shared soul present, Discord bot connected (Werner_Amvara), CPU window created and shown, Ollama configuration and connection successful, Monitor checks (amvara, mix-online, prod.cometa, app-monitor UP). Manual checks (menu bar click, `--cpu`/`-vv` in chat) left to human.

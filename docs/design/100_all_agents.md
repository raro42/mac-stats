...
- ~~Implement more robust handling for MCP server errors.~~ **Done:** Error-handling § in docs/010_mcp_agent.md; one retry for transient errors in mcp/mod.rs (list_tools, call_tool). See agents/006-feature-coder/FEATURE-CODER.md.
- ~~Improve the user interface for scheduling tasks.~~ **Done:** Settings → Schedules tab (list, add cron or one-shot, remove); Tauri commands `list_schedules`, `add_schedule`, `add_schedule_at`, `remove_schedule`. See docs/009_scheduler_agent.md and agents/006-feature-coder/FEATURE-CODER.md.
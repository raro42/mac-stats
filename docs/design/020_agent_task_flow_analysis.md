...
| **RUN_JS** | `RUN_JS: <JavaScript code>` | Execute JavaScript on the host via Node (tool loop). | **CPU window**: frontend may run code in WebView. **Discord/agent**: `delegation_tool_dispatch::handle_run_js` → `run_js_via_node` unless **`runJsEnabled`** is false in `~/.mac-stats/config.json` (stable refusal, no Node spawn). Orthogonal to **BROWSER_*** (`browserToolsEnabled`). |
...
- ~~**Documentation**: Update for clarity and completeness.~~ **Done:** Tool table above completed (RUN_JS, PERPLEXITY_SEARCH, RUN_CMD implementation details); See also added for full list. See agents/006-feature-coder/FEATURE-CODER.md.

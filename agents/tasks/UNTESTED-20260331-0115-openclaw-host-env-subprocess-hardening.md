...
- **Coder (UTC):** 2026-03-31 — **FEATURE-CODER** (`agents/006-feature-coder/FEATURE-CODER.md`). Assigned `agents/tasks/FEAT-20260331-0115-openclaw-host-env-subprocess-hardening.md` **absent**; used **`UNTESTED-…` → `WIP-…` → `UNTESTED-…`** (same stem). **Implementation:** `security::host_exec_env` + §4 call sites (**no Rust changes this run** — already in tree). **§5 `rg`:** all listed paths reference `apply_host_exec_env_hardening` / `apply_host_exec_env_hardening_tokio`. **§6 Testing:** Section 6 below (tester one-liner, smoke, manual). **Verification (coder re-run):** `cargo check`; `cargo test host_exec_env` (**3**); `cargo test pipeline_date_wc` (**1**); `cargo test --lib` (**878** passed); full `cargo test` — **pass** (doc-tests: **1** ignored). **Handoff:** `agents/tasks/WIP-…` → `agents/tasks/UNTESTED-20260331-0115-openclaw-host-env-subprocess-hardening.md` (this run).
...
**FEATURE-CODER task-file lifecycle (same stem `20260331-0115-openclaw-host-env-subprocess-hardening`):** `agents/tasks/FEAT-….md` → rename to `WIP-….md` while coding → add/keep this section → rename to `UNTESTED-….md` for tester handoff. If **`FEAT-…` does not exist** but **`UNTESTED-…` does** (coder pickup), rename **`UNTESTED-…` → `WIP-…`** first, then finish work and **`WIP-…` → `UNTESTED-…`** again for handoff.

**Assigned `FEAT-…` path missing:** When instructions name `agents/tasks/FEAT-20260331-0115-….md` but that file is absent and `agents/tasks/UNTESTED-….md` exists (same stem), use the **`UNTESTED → WIP → UNTESTED`** flow above — do not substitute a different FEAT task.

**On-disk handoff path:** after the coder **`WIP → UNTESTED`** rename, this task lives at `agents/tasks/UNTESTED-20260331-0115-openclaw-host-env-subprocess-hardening.md` (same stem as `FEAT-…` / `WIP-…`).
...
**Result:** all passed (`host_exec_env`: 3; `pipeline_date_wc`: 1; `cargo test --lib`: **878**; full `cargo test` including doc-tests — **pass**, 1 ignored). Recorded immediately before **`WIP-…` → `UNTESTED-20260331-0115-openclaw-host-env-subprocess-hardening.md`** handoff (FEATURE-CODER; assigned `FEAT-…` absent; lifecycle **`UNTESTED→WIP→UNTESTED`** per §6). Re-verified on **2026-03-31** (same counts).

...
**Instruction revision note:** A prior run flagged **Testing instructions** or the **stated environment** as defective (not a mac-stats implementation failure). This markdown body is the only authoritative spec: follow **Verification commands** below exactly, not snippets copied from **`CLOSED-*`** history (those may use wrong paths such as top-level **`src/`** or a shortened **`rg`** that omits required literals). **Latest repair (2026-03-30, twelfth pass — coder):** **Copy-paste order** step **0b** now explicitly covers **BLOCK: A1 or A2** when the probe prints **no** **`cd`** line (already at repo root): you still run **`test -f src-tauri/Cargo.toml && echo OK`** before Preflight — previously the table only mentioned **`cd` + test**, which some testers read as “skip 0b entirely” and then questioned whether step **0b** was mandatory. Added **Ultra-short path (bash)** under **Testing instructions** (short numbered list: shell → **0** → **0b** → **1** → Self-check → **2**). **Retest quickstart R3** cross-linked to the updated **0b** rule. **Prior (eleventh pass):** **Tester cheat sheet**; **TL;DR** §3 **B** vs **A1/A2**; **R1–R5**; Preflight **`$REPO_ROOT` ≠ `pwd`** note; defect **16**.
...
### Ultra-short path (bash — use if the long sections disagree)

1. **Shell:** Use **`bash`** (or **`zsh`**) for **every** paste — not **fish** (see **Tester quick gate** → **Shell for step 0**).
2. **Step 0:** Paste **Tester quick gate** (probe). Note **`BLOCK:`** (**B** vs **A1 or A2** vs **none**).
3. **Step 0b:** If **`BLOCK: B`** — **do nothing** for 0b (no repo-root **`test -f src-tauri/…`**). If **`BLOCK: A1 or A2`** — if the probe printed a **`cd '…'`** line, run it, **then** run **`test -f src-tauri/Cargo.toml && echo OK`**; if there was **no** **`cd`** line (you are already at mac-stats repo root), run **`test -f src-tauri/Cargo.toml && echo OK`** from **`pwd`** anyway (confirms repo root before Preflight).
4. **Step 1:** **Preflight** once — for **`BLOCK: B`** use **git** variant only; for **`BLOCK: A1 or A2`** use **git** or **no-git** per **Preflight (required)**.
5. **Self-check:** Run **exactly one** row from **Self-check before step 2** (matches your **BLOCK**).
6. **Step 2:** Paste **one** full block **A1**, **A2**, or **B** from **Verification commands** (**`set -e`** through last **`rg`**).

### Tester cheat sheet (read this block first)

| Rule | **BLOCK: A1 or A2** (repo-root verification) | **BLOCK: B** (`src-tauri/` crate root) |
|------|-----------------------------------------------|----------------------------------------|
| **Shell** | Use **`bash`** (or **`zsh`**) for **every** pasted script — **not** **fish** (see **Shell compatibility**). | Same |
| **After probe** | **`BLOCK: A1 or A2`:** run any printed **`cd '…'`**, **then** **`test -f src-tauri/Cargo.toml && echo OK`** (**always**, including when there was **no** **`cd`** line — already at repo root). | **`BLOCK: B`:** **Do not** run **`test -f src-tauri/Cargo.toml`** — you stay in **`src-tauri/`**; that path would mean `src-tauri/src-tauri/…` and **fails by design**. |
| **Preflight (step 1)** | **Git** or **no-git** variant (no-git **only** if **`pwd`** is already mac-stats **repo root**). | **Git variant only** (walk-up still finds the root for **`test -f "$REPO_ROOT/…"`**). **Never** no-git from crate root. |
| **Before verification paste** | Run **Self-check** row **A1/A2** in **Self-check before step 2**. | Run **Self-check** row **B** ( **`Cargo.toml`** + **`src/commands/turn_lifecycle.rs`** at crate root). |
| **Step 2** | Paste block **A1** *or* **A2** in full. | Paste block **B** in full **without** **`cd`** to repo root — **`rg`** paths are **`src/…`**, not **`src-tauri/src/…`**. |

**Hard rule:** Do **not** run **`cargo`** or **`rg`** from **Verification commands** until **Preflight** (step **1**) has succeeded in the **same** terminal — **even** when the probe said **`BLOCK: B`**.

### Retest quickstart (minimal decision — phases R1–R5)

These **R** phases map to **Copy-paste order** **0 / 0b / 1 / 2**; they are **not** extra shell steps beyond that table.

- **R1 — Terminal:** Open **one** terminal with **`pwd`** **anywhere inside** the mac-stats checkout (if unsure, **`cd`** to the folder that **contains** **`src-tauri/`**). If your shell is **fish**, start **`bash`** first and stay in **`bash`** for **R2–R5** (see **Tester quick gate** → **Shell for step 0**).
- **R2 — Probe:** Paste **Tester quick gate** (**Copy-paste order** step **0**). The printed **`BLOCK:`** line is the **only** selector for which block you paste in **R5**.
- **R3 — Align cwd (Copy-paste order step 0b):** If **`BLOCK: B`** — **skip** **`test -f src-tauri/Cargo.toml`**; you remain at **`src-tauri/`** for **R4–R5**. If **`BLOCK: A1 or A2`:** run the probe’s **`cd '…'`** line **if** it printed one, **then** always run **`test -f src-tauri/Cargo.toml && echo OK`** (required before **A1**/**A2** — including when there was **no** **`cd`** line because **`pwd`** was already repo root).
- **R4 — Preflight:** Run **Preflight** (step **1**) once — **git** variant whenever **`BLOCK: B`** or from any subdir under mac-stats; **no-git** variant **only** when **`pwd`** is mac-stats **repo root** (never from **`src-tauri/`** alone).
- **R5 — Verify:** Run **Self-check before step 2** (one row), then paste **exactly one** full block **A1**, **A2**, or **B** from **Verification commands** — **not** snippets from **`tasks/CLOSED-*`** for this stamp.

### Preflight variant chooser (after probe R2 / step 0 — before Preflight R4 / step 1)

| Step **0** printed | Paste for step **1** (**Preflight (required)**) |
|--------------------|---------------------------------------------------|
| **`BLOCK: B`** | **Git variant only** (walk-up / `$REPO_ROOT`). **Do not** paste the **no-git** snippet while **`pwd`** is **`src-tauri/`** — it will fail on **`test -f src-tauri/Cargo.toml`**. |
| **`BLOCK: A1 or A2`** | **Git** variant **or** **no-git** variant, per **Preflight (required)** — **no-git** is allowed **only** when **`pwd`** is already mac-stats **repo root** (after step **0b** if the probe printed a **`cd`**). |
...
### Self-check before step 2 (one command each)

Run **exactly one** row **after** Preflight succeeds and **before** pasting **Verification commands**. If a check fails, fix **`cd`** or re-run the probe — **do not** paste **A1**/**A2**/**B** yet.

| Probe printed | Run (from current `pwd`, unchanged since Preflight unless you knowingly `cd`’d) | Expect |
|---------------|----------------------------------------------------------------------------------|--------|
| **`BLOCK: A1 or A2`** | `test -f src-tauri/Cargo.toml && test -f src-tauri/src/commands/turn_lifecycle.rs && echo "OK: repo root for A1/A2"` | success |
| **`BLOCK: B`** | `test -f Cargo.toml && test -f src/commands/turn_lifecycle.rs && echo "OK: crate root for B"` | success |

**If the probe said `BLOCK: B` but the second-row check fails:** you are **not** in **`src-tauri/`** — **`cd`** to the crate root (the directory containing **`Cargo.toml`** whose **`src/commands/turn_lifecycle.rs`** exists), re-run **Preflight** (git variant), then re-check.
...
- **Step 0b — Align `pwd` / confirm repo root for A1/A2:** When the probe says **BLOCK: B**, skip **0b** (stay at **`src-tauri/`**). When the probe says **BLOCK: A1 or A2**, run any printed **`cd '…'`**, **then** run **`test -f src-tauri/Cargo.toml && echo OK`** — **also** when there was **no** **`cd`** line (already at repo root), run that **`test -f`** from **`pwd`** so you do not skip **0b** by mistake.
...
| **0b** | If probe says **BLOCK: B** — **skip** this step (stay at **`src-tauri/`**; repo-root **`test -f src-tauri/…`** fails **by design** from crate root). If probe says **BLOCK: A1 or A2** — run any printed **`cd '…'`** line first, **then** run **`test -f src-tauri/Cargo.toml && echo OK`** from **`pwd`** (**required** even when the probe printed **no** **`cd`** line because you were **already** at mac-stats repo root). |
...
3. Run **Tester quick gate** step **0** (walk-up probe; **no** `git` required). **If** the probe says **`BLOCK: A1 or A2`:** run **`cd`** if the probe printed it, **then** **`test -f src-tauri/Cargo.toml`** from **`pwd`** (**required** even when there was **no** **`cd`** line — you were already at repo root). **If** it says **`BLOCK: B`:** **do not** run that repo-root **`test`** — stay in **`src-tauri/`** for Preflight + block **B**.
...
| 7 | **Step 0b for `BLOCK: A1 or A2`:** Run any printed **`cd`**, **then** **`test -f src-tauri/Cargo.toml`** (expect success) **before** Preflight and **A1**/**A2**. If the probe printed **no** **`cd`** (already at repo root), run that **`test -f`** from **`pwd`** anyway. If that **`test`** fails, you are not at mac-stats **repo root** yet — fix **`cd`** and retry. |
...
**Fast path (experienced testers, macOS):** Same **0 → 0b → 1 → 2** as **Copy-paste order** — **no** shortcuts. **Tester quick gate** step **0** → step **0b**: **skip** only for **`BLOCK: B`**; for **`BLOCK: A1 or A2`** always run repo-root **`test -f src-tauri/Cargo.toml`** after any probe **`cd`** (or from **`pwd`** if **no** **`cd`** line) → step **1** **Preflight** always (**git** variant if **`.git`** exists or when **`BLOCK: B`**; **no-git** only at repo root and **never** from **`src-tauri/`** alone) → step **2** paste **one** full block **A1**, **A2**, or **B** (never skip leading **`cd`** / **`test -f`** lines inside the block). **Tarball / broken git:** **A2** with a real absolute path on the **`cd`** line. **Pass** = every command exits **0** and every **`rg`** prints at least one line.

**Happy path (ordered):** **0** Tester quick gate → **0b** — if **`BLOCK: A1 or A2`**: run printed **`cd`** if any, then **`test -f src-tauri/Cargo.toml`**; if **`BLOCK: B`**: **skip** that repo-root **`test`** (you stay at **`src-tauri/`**; see **Copy-paste order**) → **1** Preflight — if **`BLOCK: B`**: **git variant only**; if **`BLOCK: A1 or A2`**: **git or no-git** per **Preflight (required)** (**not** the same paste as verification block **A2**) → **Self-check before step 2** → **2** one full block **A1** or **A2** or **B** → **after bash succeeds:** **Closure checklist** (**TESTER.md** paperwork only — **not** “step 3” of the shell table; **not** another bash paste). Use the checklist only when the queue file is **`UNTESTED-…`** and you already renamed **`UNTESTED-…` → `TESTING-…`** per **TESTER.md**.
...
**Shell for step 0:** Paste the probe into **`bash`** or **`zsh`**. **Do not** paste it into **fish** (or other non-POSIX shells): the script uses **`$(dirname "$d")`** and **`if …; then …; else …; fi`**, which **fish** parses differently and can yield a **wrong `BLOCK:`** or a syntax error. Open **`bash`** first, or run **`bash -lc '…'`** with the probe body inside the quotes.
...
12. **Skipping Preflight after the probe (especially `BLOCK: B`)** — **Copy-paste order** is **0 → 0b (skip 0b only for `BLOCK: B`) → 1 → 2**. For **`BLOCK: A1 or A2`**, step **0b** is **not** optional even when the probe printed **no** **`cd`** line. Older summaries that said “paste **B** only” after the probe were **wrong**; you **must** run **Preflight** (step **1**) before **any** **A1**/**A2**/**B** paste.
...
14. **Using `tasks/CLOSED-*` (or chat logs) as the command source** — archived **`CLOSED-*`** files for this stamp may list **`rg … src/`** or omit required **`rg`** lines / **`-p mac_stats`**. That yields false “missing code” or incomplete verification. **Only** the **Verification commands** section **in this file** is authoritative for **`rg`** paths and **`cargo`** invocations.
15. **Pasting the step 0 probe into fish** — the **Tester quick gate** script is **bash/zsh** syntax. In **fish**, you may get errors or nonsense **`BLOCK:`** output. Run **`bash`** (or **`bash -lc '…'`**) for step **0**, then continue in the same **`bash`** session for steps **0b–2** if your default shell is **fish**.
16. **Running `test -f src-tauri/Cargo.toml` after `BLOCK: B`** — from **`src-tauri/`** crate root that path is **wrong** (nested `src-tauri/`). **TL;DR** §3 and **Retest quickstart R3** explicitly **skip** this when the probe chose block **B**. Run **git** Preflight, **Self-check** row **B**, then paste block **B**.
17. **Skipping step 0b for `BLOCK: A1 or A2` because the probe printed no `cd` line** — **Copy-paste order** step **0b** still requires **`test -f src-tauri/Cargo.toml && echo OK`** from **`pwd`** before Preflight. The old wording (“if the probe prints **`cd`**…”) made some testers think **0b** was **only** for the subdirectory case.
...
**`$REPO_ROOT` vs `pwd`:** The printed **`OK: mac-stats repo root = …`** path may **differ** from your shell’s **`pwd`** (e.g. you ran Preflight from **`tasks/`** or **`src-tauri/`**). That is **normal** — the snippet resolves the mac-stats root for its **`test -f "$REPO_ROOT/…"`** checks only. **Do not** interpret it as “I must **`cd "$REPO_ROOT"`** now”: if the probe said **`BLOCK: B`**, you must **stay** at **`src-tauri/`** for block **B** (see **Execution recipe** / **Tester cheat sheet**).
...
1. **(Same as table 0 / 0b / 1.)** Run **Tester quick gate** step **0** (directory probe). **If `BLOCK: B`:** skip repo-root **`test -f src-tauri/…`**, go to **Preflight**. **If `BLOCK: A1 or A2`:** run any printed **`cd`**, then **`test -f src-tauri/Cargo.toml`** (or, if **no** **`cd`** line, run that **`test -f`** from **`pwd`**). Then run **Preflight (required)**: the **git** snippet if you have a `.git` tree (can run from **`tasks/`** etc.), **or** the **no-git** snippet after you have manually **`cd`'d** to repo root — **not** the full **Verification commands → A2** block.
...
**Source of truth:** Copy **only** from this section. Do **not** reuse verification lines from **`tasks/CLOSED-20260321-2000-openclaw-hung-turn-timeout-event-gate.md`** or other archives — they may search top-level **`src/`** or skip literals required by **Acceptance criteria** §3.

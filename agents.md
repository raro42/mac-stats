...
- **Overnight harness loop (20:00–06:00 local, every 20m)**: Keep the Cursor `AGENT_LOOP_TICK_harness` shell loop running for overnight work. **Quiet during daytime** (do not tick while the human is working). If the overnight loop exits, restart it. Strategy: `docs/autoresearch/program.md`. Keep/discard gate: `scripts/autoresearch_ratchet.py`. Prefer the monitored shell loop over cron for coding ticks. **~23:00:** loop runs `scripts/overnight_git_flush.py` once to commit+push any pending safe dirty files (backstop — still commit when work finishes).

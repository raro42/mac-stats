...
- **Secrets for LaunchAgent installs**: Keep API keys in `src-tauri/.config.env` for local `cargo run`, and run `./scripts/sync-home-config-env.sh` (also called from `install-to-applications.sh`) so `REDMINE_URL` / `REDMINE_API_KEY` (and Brave/Perplexity) land in `~/.mac-stats/.config.env`. The installed app does not use the repo cwd.

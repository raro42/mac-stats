...
- **Overnight harness loop (every 20m)**: Keep the Cursor `AGENT_LOOP_TICK_harness` shell loop running whenever this session is active. If it exits or is killed (app reinstall, reboot of the agent shell), **restart it immediately** — do not wait to be asked. Prefer the monitored shell loop over cron: cron cannot wake the Cursor agent; only the tick echo does. Optional cron/LaunchAgent is fine for *non-agent* jobs (e.g. `digest_agent_runs.py` alone), not for coding ticks.

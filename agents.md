...
- **Overnight harness loop (20:00–06:00 local, every 20m)**: Keep `python3 -u scripts/run_overnight_harness_loop.py` running. It prints `AGENT_LOOP_TICK_harness` and **spawns the Cursor `agent` CLI** (`-p --force --trust`) so experiments run without an IDE consumer. **Quiet during daytime** (no agent spawn). If the loop exits, restart it. Strategy: `docs/autoresearch/program.md`. Keep/discard gate: `scripts/autoresearch_ratchet.py`. Logs: `~/.mac-stats/improvements/overnight_agent.log`. **~23:00:** loop runs `scripts/overnight_git_flush.py` once as a backstop (still commit when work finishes).

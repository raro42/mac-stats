...
- **Push when reasonable**: Do **not** wait for an explicit “please push.” After local commits that are ready to share (especially overnight/harness version ships), push to `origin` (`git push origin HEAD` / current branch). Skip push only when the build is known-broken, secrets are involved, the user said local-only, or a force-push to `main`/`master` would be required. **GitHub Releases**: do **not** wait to be asked — when `Cargo.toml` is meaningfully ahead of GitHub Latest, run `python3 scripts/maybe_cut_github_release.py` (overnight ~23:00 flush also runs it). Homebrew cask SHA is a follow-up after the DMG asset exists.
- **No uncommitted leftovers**: When a task is finished, **commit and push** — do not leave finished work dirty. Nightly ~23:00 harness flush (`scripts/overnight_git_flush.py`) is a backstop. Rule: `.cursor/rules/no-uncommitted-leftovers.mdc`.
- **Werner / Discord schedules that need git**: Do **not** put bare “commit+push” as the only action in a schedule task without a `SKILL:` or `CURSOR_AGENT:` path. Discord’s fast lane still refuses *casual* chat “commit and push” (safety). Weekly skills must say `SKILL: …` and finish with `CURSOR_AGENT: in ~/projects/mac-stats commit and push` so Werner does not stop at the safety refusal.
- **Secrets for LaunchAgent installs**: Keep API keys in `src-tauri/.config.env` for local `cargo run`, and run `./scripts/sync-home-config-env.sh` (also called from `install-to-applications.sh`) so `REDMINE_URL` / `REDMINE_API_KEY` (and Brave/Perplexity) land in `~/.mac-stats/.config.env`. The installed app does not use the repo cwd. **Never commit** `.config.env`, `.env`, credentials, or private keys (user-facing docs omit this agent reminder).
- **CHANGELOG.md**: Keep a Changelog is **user-facing** only. Do **not** append OpenClaw-reviewer ticks, tester CLOSED/TESTING paperwork, or digest hashes. Weekly review: Mondays ~10:00 (Werner schedule + `changelog-weekly-review` skill); policy in `docs/040_changelog_hygiene.md`.
- **Repo quality (weekly)**: Fridays ~10:00 — `SKILL: quality-weekly-review` + `python3 scripts/scan_repo_quality.py`. Catch dead root scaffolding (npm leftovers, moved dirs, stale links) without waiting for a human prompt. Policy: `docs/044_repo_quality_hygiene.md`; agent home: `agents/007-quality-monitor/`.
- **Overnight harness loop (20:00–06:00 local, every 20m)**: Keep `python3 -u scripts/run_overnight_harness_loop.py` running via **LaunchAgent** (`./scripts/install-overnight-harness-launchagent.sh` → `com.raro42.mac-stats-overnight-harness`, `KeepAlive`). It prints `AGENT_LOOP_TICK_harness` and **spawns the Cursor `agent` CLI** (`-p --force --trust`) so experiments run without an IDE consumer. **Quiet during daytime** (no agent spawn). If the loop exits, restart it (or re-run the install script). Strategy: `docs/autoresearch/program.md`. Keep/discard gate: `scripts/autoresearch_ratchet.py`. Logs: `~/.mac-stats/improvements/overnight_agent.log` + `overnight_harness_loop.stdout.log`. Morning surprise: live under `~/.mac-stats/improvements/`, **also** archive to `docs/ops/morning-surprises/` (`scripts/archive_morning_surprise.py`) and commit. **~23:00:** loop runs `scripts/overnight_git_flush.py` once as a backstop (still commit when work finishes).
- **Agent Ops UI**: Command Center overview (schedules / live / knowledge / recent chats) is the operator surface — see `docs/041_ui_command_center.md`. Weekly UI polish: Wednesdays ~11:00 (`ui-weekly-review` skill); do not conflate with harness overnight ticks.

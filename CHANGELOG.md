...
### Changed
- **HTML cleaning: Supplemental Punctuation + Aegean / Phoenician word separators (FETCH_URL)** — `collapse_whitespace()` maps U+2E31–U+2E33 (word separator middle dot, three-dot punctuation, raised dot), U+10100–U+10101 (Aegean word separator line/dot), and U+1091F (Phoenician word separator) to ASCII space; all are Po and not Rust whitespace, so typographic or scholarly HTML can otherwise glue Latin tokens. Tests `supplemental_punctuation_word_separator_dots_separate_words`, `aegean_and_phoenician_word_separator_marks_separate_words` in `commands/html_cleaning.rs`. (FEAT-D58.)

...
### Changed
- **HTML cleaning: Halfwidth forms light vertical, arrows, square, circle (FETCH_URL)** — `collapse_whitespace()` maps U+FFE8–U+FFEE (halfwidth forms light vertical; halfwidth left/up/right/down arrows; halfwidth black square; halfwidth white circle; So/Sm) to ASCII space; none are Rust whitespace, so JIS / compat typography HTML can otherwise glue Latin tokens for `split_whitespace()`. U+FFE3 FULLWIDTH MACRON (Sk) remains unmapped between the earlier currency arm and this tail. Test `halfwidth_forms_light_vertical_arrows_square_circle_separate_words` in `commands/html_cleaning.rs`. (FEAT-D127.)

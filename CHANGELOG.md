...
- **HTML cleaning (FETCH_URL)** — `collapse_whitespace()` maps Latin-1 cent / pound / currency / yen signs U+00A2–U+00A5 (`Sc`) to ASCII space; not Rust whitespace. U+00A6 BROKEN BAR (`Sm`) and U+00A8 DIAERESIS (`Sk`) stay unmapped. Tests `latin1_currency_signs_u00a2_through_u00a5_separate_words`, `latin1_broken_bar_sm_u00a6_and_diaeresis_sk_u00a8_stay_unmapped` in `commands/html_cleaning.rs`. (FEAT-D223.)

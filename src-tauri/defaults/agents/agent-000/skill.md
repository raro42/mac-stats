...
- **MEMORY:** add <lesson> — short lesson/preference. **MEMORY: save <slug>** then full verbatim body on following lines — when the user asks to save a plan, itinerary, list, or exact details for later (never summarize). **MEMORY: read note:<slug>** — recall a saved note. MEMORY_APPEND: <lesson> still works as add. Optional agent:<slug> prefix.

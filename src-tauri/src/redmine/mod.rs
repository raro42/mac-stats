...
run ./scripts/sync-home-config-env.sh then restart mac-stats)"
    ...
    if !status.is_success() {
        let body_snippet = crate::logging::ellipse(&body_text, 500);
        info!(
            "Redmine API {} {} → {} body: {}",
            method_upper, path, status, body_snippet
        );
        let code = status.as_u16();
        if code == 401 {
            return Err(format!(
                "Redmine API 401: Unauthorized. Check that REDMINE_API_KEY is valid and enabled in Redmine (My account → API access). {}",
                body_snippet
            ));
        }
        if code == 404 {
            return Err(format!(
                "Redmine API 404: Not Found. Check issue ID, project ID, or path. {}",
                body_snippet
            ));
        }
        if code == 422 {
        ...
        return Err(format!("Redmine API {}: {}", status, body_snippet));
    }

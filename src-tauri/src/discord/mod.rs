...
pub(super) async fn run_discord_ollama_router(
    ...
    // `/failed` — recent ok=false turns from runs.jsonl, no Ollama.
    if crate::commands::harness_ops::looks_like_failed_runs_request(&content) {
        let days = crate::commands::harness_ops::parse_insights_days(&content);
        let report = crate::commands::harness_ops::format_failed_runs_gateway(days);
        if let Err(e) = new_message.channel_id.say(&ctx, report).await {
            error!("Discord: failed to send failed-runs report: {}", e);
        }
        return;
    }
    ...
    // `/lite` — recent lite-lane turns from runs.jsonl, no Ollama.
    if crate::commands::harness_ops::looks_like_lite_runs_request(&content) {
        let days = crate::commands::harness_ops::parse_insights_days(&content);
        let report = crate::commands::harness_ops::format_lite_runs_gateway(days);
        if let Err(e) = new_message.channel_id.say(&ctx, report).await {
            error!("Discord: failed to send lite-runs report: {}", e);
        }
        return;
    }
    ...
    if crate::commands::harness_ops::looks_like_status_request(&content) {
        ...
        if let Err(e) = new_message.channel_id.say(&ctx, report).await {
            error!("Discord: failed to send status: {}", e);
        }
        return;
    }
    ...
    // `/sessions` — Agent Ops Live/Files list, no Ollama.
    if crate::commands::harness_ops::looks_like_sessions_request(&content) {
        let filter = crate::commands::harness_ops::parse_sessions_list_filter(&content);
        let report = crate::commands::harness_ops::format_sessions_gateway(filter);
        if let Err(e) = new_message.channel_id.say(&ctx, report).await {
            error!("Discord: failed to send sessions list: {}", e);
        }
        return;
    }

    // `/knowledge` — Agent Ops Discord/Core list, no Ollama.
    if crate::commands::harness_ops::looks_like_knowledge_request(&content) {
        let filter = crate::commands::harness_ops::parse_knowledge_list_filter(&content);
        let report = crate::commands::harness_ops::format_knowledge_gateway(filter);
        if let Err(e) = new_message.channel_id.say(&ctx, report).await {
            error!("Discord: failed to send knowledge list: {}", e);
        }
        return;
    }
    ...
    // `/logs` — Debug Log Error/Warn list, no Ollama.
    if crate::commands::harness_ops::looks_like_debug_log_request(&content) {
        let filter = crate::commands::harness_ops::parse_debug_log_list_filter(&content);
        let report = crate::commands::harness_ops::format_debug_log_gateway(filter);
        if let Err(e) = new_message.channel_id.say(&ctx, report).await {
            error!("Discord: failed to send debug log list: {}", e);
        }
        return;
    }

...
/// Pick one open task path to work on (first by filename order). Returns None if no open tasks.
fn pick_one_open_task() -> Option<std::path::PathBuf> {
    let list = crate::task::list_open_and_wip_tasks().ok()?;
    let open: Vec<_> = list.into_iter().filter(|(_, s, _)| s == "open").map(|(p, _, _)| p).collect();
    open.into_iter().next()
}

/// Run one review cycle: close stale WIPs, then work on one open task if any.
async fn run_review_once() {
    ...
    info!("Task review: starting work on open task '{}'", name);
    match crate::commands::ollama::run_task_until_finished(path.clone(), 10).await {
        Ok(reply) => {

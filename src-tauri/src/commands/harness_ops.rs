...
/// True for short “how big is digest.md / latest.md size…” asks.
/// Stat only on `latest.md` — does not steal `digest size` (latest.json) / age / open / refresh.
pub fn looks_like_digest_md_size_request(content: &str) -> bool {
    let n = normalize_operator_command(content);
    if n.chars().count() > 56 {
        return false;
    }
    ...
    // Require explicit latest.md / digest.md — do not steal bare `digest size` (latest.json).
    ...
    matches!(
        ...
        "digest.md size"
            ...
            | "latest.md size"
            | "latest md size"
            | "how big is digest.md"
            | "how big is latest.md"
            | "how big is the digest.md"
            | "how big is the latest.md"
            | "how large is digest.md"
            | "how large is latest.md"
            | "how large is the digest.md"
            | "how large is the latest.md"
            | "digest.md bytes"
            | "latest.md bytes"
            | "digest markdown size"
            | "md digest size"
    ) || (n.contains("size") && md_ctx)
        || (n.contains("big") && md_ctx)
        || (n.contains("large") && md_ctx)
        || (n.contains("bytes") && md_ctx)
        || ((n.contains(" mb") || n.contains(" kb") || n.contains(" gi")) && md_ctx)
}

/// Zero-LLM digest `latest.md` file size (stat only; no digester spawn / open dump).
pub fn format_digest_md_size_gateway() -> String {
    let path = digest_md_path();
    if !path.exists() {
        return "**Digest.md:** no `latest.md` yet · run `/digest` first · `digest size` for `latest.json`."
            .to_string();
    }
    match std::fs::metadata(&path).map(|m| m.len()) {
        Ok(0) => {
            "**Digest.md:** empty `latest.md` · `/digest` to refresh · `digest size` for `latest.json`."
                .to_string()
        }
        Ok(bytes) => {
...
/// Stat only — does not list, count, prune, or steal path/age lanes.
pub fn looks_like_runs_size_request(content: &str) -> bool {
    let n = normalize_operator_command(content);
    if n.chars().count() > 56 {
        return false;
    }
    ...
    matches!(
        ...
        "runs size"
            ...
            | "runs log size"
            | "runs file size"
            | "how big is runs.jsonl"
            | "how big is the runs.jsonl"
            | "how big is runs jsonl"
            | "how big is the runs file"
            | "how big is runs file"
            | "how big is the run log"
            | "how big is run log"
            | "how big are runs"
            | "how large is runs.jsonl"
            | "how large is the runs file"
            | "how large is runs"
            | "runs.jsonl bytes"
            | "runs file bytes"
            | "run log bytes"
    ) || (n.contains("size") && runs_ctx)
        || (n.contains("big") && runs_ctx)
        || (n.contains("large") && runs_ctx)
        || (n.contains("bytes") && runs_ctx)
        || ((n.contains(" mb") || n.contains(" kb") || n.contains(" gi")) && runs_ctx)
}

/// Zero-LLM runs.jsonl file size (stat only; no list/count/prune).
pub fn format_runs_size_gateway() -> String {
    let path = crate::commands::run_telemetry::runs_jsonl_path();
    if !path.exists() {
        return "**Runs:** no `runs.jsonl` yet · `/insights` after the first chat turn · `runs path` for the file.".to_string();
    }
    match std::fs::metadata(&path).map(|m| m.len()) {
        Ok(0) => {
            "**Runs:** empty `runs.jsonl` · `runs path` for the file.".to_string()
        }
        Ok(bytes) => {
            let label = crate::commands::disk_cleanup::format_bytes(bytes);
            format!(
                "**Runs:** **{label}** on disk · `/insights` or Agent Ops → Runs · `runs path` for the file · `runs age` for last write."
            )
        }
...
pub fn looks_like_runs_age_request(content: &str) -> bool {
    ...
    // Do not steal path / size / count / list / insights / failed / slow / lane dumps.
    // Path/size steal is keyword-only (no looks_like_runs_* nesting) to avoid mutual recursion.
...
pub fn looks_like_downloads_organizer_rules_path_request(content: &str) -> bool {
    ...
    let pathish = n.contains("path")
        ...
        || n.contains("file")
        || n.contains("md")
        || n.contains("downloads-organizer-rules.md")
        || n.contains("downloads-organizer-rules");
    matches!(
        n.as_str(),
        "downloads organizer rules path"
            | "downloads organizer rule path"
            | "downloads-organizer-rules"
            | "downloads-organizer-rules.md"
            | "downloads-organizer-rules.md path"
            | "downloads-organizer-rules path"
            | "downloads-organizer-rules file"
            | "downloads_organizer_rules"
            | "downloads_organizer_rules.md"
            | "downloads_organizer_rules path"
            ...
            | "organizer-rules path"
            | "organizer rules"
            | "organizer rules file"
            | "organizer rules file path"
            | "organizer rules md"
            | "downloads organizer rules"
            | "downloads organizer rules file"
            | "downloads organizer rules file path"
            | "where is downloads-organizer-rules"
            | "where is downloads-organizer-rules.md"
            | "where is the downloads-organizer-rules file"
            | "where is downloads organizer rules"
            | "where is the downloads organizer rules file"
            | "where are downloads organizer rules"
            | "where is organizer rules"
            | "where is the organizer rules file"
            | "where are organizer rules"
    ) || (rules_ctx && pathish)
}

/// Zero-LLM downloads-organizer-rules.md path (config only; no list/edit/run).
pub fn format_downloads_organizer_rules_path_gateway() -> String {
...
pub fn looks_like_downloads_organizer_state_path_request(content: &str) -> bool {
    ...
    // String-only sibling excludes (do not nest looks_like_*_path_request — exponential).
    if n.contains("downloads-organizer-rules")
        || n.contains("downloads_organizer_rules")
        || n.contains("downloads organizer rules")
        || n.contains("downloads organizer rule")
        || n.contains("organizer-rules")
    ...
    let pathish = n.contains("path")
        ...
        || n.contains("file")
        || n.contains("json")
        || n.contains("downloads-organizer-state.json")
        || n.contains("downloads-organizer-state");
    matches!(
        n.as_str(),
        "downloads organizer state path"
            | "downloads-organizer-state"
            | "downloads-organizer-state.json"
            | "downloads-organizer-state.json path"
            | "downloads-organizer-state path"
            | "downloads-organizer-state file"
            | "downloads_organizer_state"
            | "downloads_organizer_state.json"
            | "downloads_organizer_state path"
            | "organizer state path"
            | "organizer-state"
            | "organizer-state.json"
            | "organizer-state path"
            | "organizer state"
            | "organizer state file"
            | "organizer state file path"
            | "organizer state json"
            | "downloads organizer state"
            | "downloads organizer state file"
            | "downloads organizer state file path"
            | "where is downloads-organizer-state"
            | "where is downloads-organizer-state.json"
            | "where is the downloads-organizer-state file"
            | "where is downloads organizer state"
            | "where is the downloads organizer state file"
            | "where is organizer state"
            | "where is the organizer state file"
    ) || (state_ctx && pathish)
}

/// Zero-LLM downloads-organizer-state.json path (config only; no dump/edit/run).
pub fn format_downloads_organizer_state_path_gateway() -> String {
...
pub fn looks_like_skill_md_path_request(content: &str) -> bool {
    ...
    // String-only sibling excludes (do not nest looks_like_*_path_request — exponential).
    if n.contains("skills path")
        || n.contains("skills folder")
        || n.contains("skills directory")
        || n.contains("skills dir")
        || n.contains("skills location")
        || n.contains("skills home")
        || n == "skill path"
        || n == "skill folder"
        || n == "skill directory"
        || n == "skill dir"
    ...
    let skill_ctx = n.contains("skill.md")
        || n.contains("skill file")
        || n.contains("skill md")
        || n == "skill"
        || n == "where is skill"
        || n == "where is the skill"
        || n == "where is skill.md"
        || n == "where is the skill.md"
        || n == "where is the skill file"
        || n == "where is skill file"
        || (n.contains("skill")
            && !n.contains("skills")
            && (n.contains("path")
                || n.contains("where")
    ...
    // Bare "skill path" stays on the skills/ directory lane (v0.1.819).
    if n == "skill path"
        || n == "skill folder"
        || n == "skill directory"
        || n == "skill dir"
    ...
    matches!(
        ...
        "skill.md"
            ...
            | "where is the skill"
            | "where is skill.md"
            | "where is the skill.md"
            | "where is skill file"
            | "where is the skill file"
            | "where is the skill.md file"
            | "agent skill.md"
            | "agent skill.md path"
            | "agent skill file"
...
/// Zero-LLM per-agent skill.md path (config only; no dump/edit).
...
pub fn looks_like_downloads_organizer_ready_request(content: &str) -> bool {
    ...
    // Path-only rules/state asks stay on the downloads-organizer-rules (and future state) lanes.
...
pub fn try_operator_instant_reply(content: &str) -> Option<String> {
    ...
    // downloads-organizer-rules.md before /downloads Ready (path-only asks).
    ...
    // runs.jsonl size before age/path (stat only; no list/count).
    if looks_like_runs_size_request(content) {
        return Some(format_runs_size_gateway());
    }
...
fn is_insights_slowest_noise(lane: &str, wall_ms: u64, tools: &[String], question: &str) -> bool {
    ...
    if looks_like_downloads_organizer_state_path_request(question) {
        return true;
    }
    ...
    // Read-only runs.jsonl size asks (v0.1.870) — stat only; no list/count.
    if looks_like_runs_size_request(question) {
        return true;
    }
...
mod tests {
    ...
    fn downloads_organizer_rules_path_request_detected() {
        ...
        let reply = try_operator_instant_reply("where is downloads-organizer-rules.md")
            .expect("downloads-organizer-rules path instant");
    ...
    fn downloads_organizer_state_path_request_detected() {
        ...
        let reply = try_operator_instant_reply("where is downloads-organizer-state.json")
            .expect("downloads-organizer-state path instant");

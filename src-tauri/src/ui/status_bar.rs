...
pub fn build_status_text(metrics: &SystemMetrics) -> String {
    if Config::menu_bar_compact() {
        // Default: CPU + SSD (+ cached °C when the window/SMC path has already filled TEMP_CACHE).
...
pub fn make_attributed_title(text: &str) -> Retained<NSMutableAttributedString> {
    ...
    unsafe {
        ...
        // (Fair/Serious/Critical); LPM green; SSD/RAM amber (≥85%). Heat from NSProcessInfo.
        ...
        for (i, line) in lines.iter().enumerate() {
                ...
                && line_utf16 > 0
            {
                ...
                let cue_color = if is_lpm_cue {
                    &*lpm_color
                } else if is_ssd_warn || is_ram_warn {
                    &*heat_serious_color // amber — power-strip SSD/RAM ≥85%
                } else if is_heat_cue {
                    match heat_state {
                        Some("Fair") => &*heat_fair_color,
                        Some("Critical") => &*alert_color,
                        _ => &*heat_serious_color, // Serious (and unknown)
                    }
                } else {
                    &*alert_color
                };

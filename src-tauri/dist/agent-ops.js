...
function runsRowIsSlow(r) {
    const wall = Number(r?.wall_ms);
    return Number.isFinite(wall) && wall >= OPS_RUNS_SLOW_MS;
}

function runsRowIsFail(r) {
    return r?.ok === false;
}
...
function paintOpsRunsLaneChips() {
    ...
    wrap.querySelectorAll('[data-ops-runs-lane]').forEach((btn) => {
        ...
        if (key === 'instant') {
            ...
            btn.classList.toggle('has-hits', slowN > 0);
        } else if (key === 'fail') {
...
function preferOpsRunsLaneFromOverview() {
    ...
    const failN = recent.filter((r) => runsRowIsFail(r)).length;
    if (failN > 0) {
        setOpsRunsLaneFilter('fail');
        return;
    }
    const slowN = recent.filter((r) => runsRowIsSlow(r)).length;
    if (slowN > 0) {
        setOpsRunsLaneFilter('slow');
        return;
    }
    const directN = recent.filter((r) => runsRowMatchesLane(r, 'direct')).length;
    if (directN > 0) {
...
/** Parse MCP feature-health (get_feature_health probe; Agent Ops attention glance). */
function parseOpsMcpHealth(mcp) {
    const st = String(mcp?.status || '').toLowerCase();
    const msg = String(mcp?.message || '').trim();
    let wash = 'ok';
    if (st === 'notconfigured') wash = 'warn';
    else if (st === 'degraded') wash = 'warn';
    else if (st === 'unavailable') wash = 'bad';
    ...
    let glanceLine = '';
    if (wash === 'warn' && st === 'notconfigured') {
        glanceLine = 'MCP · Not set · add URL or stdio';
    } else if (wash === 'bad') {
        const bit = msg ? msg.slice(0, 24) : 'check config';
        glanceLine = `MCP · Unavailable · ${bit}`;
    } else if (wash === 'warn' && st === 'degraded') {
        const bit = msg ? msg.slice(0, 28) : 'check server';
        glanceLine = `MCP · Degraded · ${bit}`;
    } else if (wash === 'warn' && msg) {
        glanceLine = `MCP · ${st} · ${msg.slice(0, 24)}`;
    } else if (wash === 'warn') {
        glanceLine = 'MCP · check config';
    }
    return { st, msg, wash, glanceLine };
}

/**
 * MCP Not set/Unavailable/Degraded attention glance under Browser (feature_health parity).
 * Visible on every Agent Ops tab when MCP probe is not ok.
 */
function ensureOpsMcpAttentionGlance() {
    ensureOpsBrowserAttentionGlance();
    ...
    const anchor =
        ...
        health;
    if (!anchor) return null;
    let glance = document.getElementById('ops-mcp-attention-glance');
    if (!glance) {
        glance = document.createElement('div');
        glance.id = 'ops-mcp-attention-glance';
        glance.className = 'ops-mcp-attention-glance';
        glance.hidden = true;
        glance.innerHTML = '<span id="ops-mcp-attention-glance-text"></span>';
        anchor.insertAdjacentElement('afterend', glance);
        wireOpsMcpAttentionGlanceClick(glance);
    } else if (browserAtt && glance.previousElementSibling !== browserAtt) {
        browserAtt.insertAdjacentElement('afterend', glance);
    } else if (!browserAtt && braveAtt && glance.previousElementSibling !== braveAtt) {
        braveAtt.insertAdjacentElement('afterend', glance);
    }
    return glance;
}

function applyOpsMcpAttentionGlanceState() {
    const glance = ensureOpsMcpAttentionGlance();
    if (!glance) return;
    const text = document.getElementById('ops-mcp-attention-glance-text');
    if (agentOpsCollapsed) {
        glance.hidden = true;
        glance.classList.remove('has-warn', 'has-bad', 'has-not-set');
        return;
    }
    const parsed = parseOpsMcpHealth(opsMcpHealthCache);
    if (parsed.wash === 'ok' || !parsed.glanceLine) {
        glance.hidden = true;
        glance.classList.remove('has-warn', 'has-bad', 'has-not-set');
        return;
    ...
    glance.tabIndex = 0;
    if (parsed.st === 'notconfigured') {
        glance.title = 'MCP needs an HTTP/SSE URL or stdio cmd|args — click to open Settings';
        glance.setAttribute(
            'aria-label',
            'MCP not configured — click to open Settings Credentials'
        );
    } else {
        glance.title = 'MCP probe failed — click to check URL or stdio in Settings';
        glance.setAttribute(
            'aria-label',
            `${parsed.glanceLine} — click to open Settings Credentials`
        );
...
function activateOpsMcpAttentionGlance() {
    if (agentOpsCollapsed) applyOpsCollapsed(false);
    document.getElementById('settings-btn')?.click();
    requestAnimationFrame(() => {
        setTimeout(() => {
...
function wireOpsMcpAttentionGlanceClick(glance) {
    if (!glance || glance.dataset.opsMcpAttentionWired === '1') return;
    glance.dataset.opsMcpAttentionWired = '1';
    const activate = () => activateOpsMcpAttentionGlance();
    glance.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        activate();
...
/** Parse Signal feature-health (get_feature_health probe; Agent Ops attention glance). */
function parseOpsSignalHealth(signal) {
    const st = String(signal?.status || '').toLowerCase();
    const msg = String(signal?.message || '').trim().toLowerCase();
    let wash = 'ok';
    if (st === 'notconfigured') wash = 'warn';
    else if (st === 'degraded') wash = 'warn';
    else if (st === 'unavailable') wash = 'bad';
    ...
    let glanceLine = '';
    if (wash === 'warn' && st === 'notconfigured') {
        glanceLine = 'Signal · Not wired · REST API pending';
    } else if (wash === 'warn' && st === 'degraded' && msg.includes('channel')) {
        const m = String(signal?.message || '').match(/(\d+)\s+channel/);
        const n = m ? m[1] : '?';
        glanceLine = `Signal · Partial · ${n} channel(s) · REST API pending`;
    } else if (wash === 'warn' && st === 'degraded') {
        const bit = msg ? msg.slice(0, 28) : 'REST API pending';
        glanceLine = `Signal · Partial · ${bit}`;
    } else if (wash === 'bad') {
        const bit = msg ? msg.slice(0, 24) : 'check config';
        glanceLine = `Signal · Unavailable · ${bit}`;
    } else if (wash === 'warn' && msg) {
        glanceLine = `Signal · ${st} · ${msg.slice(0, 24)}`;
    } else if (wash === 'warn') {
        glanceLine = 'Signal · Not wired · REST API pending';
    }
    return { st, msg, wash, glanceLine };
...
/**
 * Signal Not wired/Partial attention glance under Slack (feature_health parity).
 * Visible on every Agent Ops tab when Signal probe is not ok (REST API not wired yet).
 */
function ensureOpsSignalAttentionGlance() {
    ensureOpsSlackAttentionGlance();
    const slackAtt = document.getElementById('ops-slack-attention-glance');
    const telegramAtt = document.getElementById('ops-telegram-attention-glance');
    const mastodonAtt = document.getElementById('ops-mastodon-attention-glance');
    const perplexityAtt = document.getElementById('ops-perplexity-attention-glance');
    const cursorAtt = document.getElementById('ops-cursor-attention-glance');
    ...
    const anchor =
        ...
        health;
    if (!anchor) return null;
    let glance = document.getElementById('ops-signal-attention-glance');
    if (!glance) {
        glance = document.createElement('div');
        glance.id = 'ops-signal-attention-glance';
        glance.className = 'ops-signal-attention-glance';
        glance.hidden = true;
        glance.innerHTML = '<span id="ops-signal-attention-glance-text"></span>';
        anchor.insertAdjacentElement('afterend', glance);
        wireOpsSignalAttentionGlanceClick(glance);
    } else if (slackAtt && glance.previousElementSibling !== slackAtt) {
        slackAtt.insertAdjacentElement('afterend', glance);
    } else if (!slackAtt && telegramAtt && glance.previousElementSibling !== telegramAtt) {
        telegramAtt.insertAdjacentElement('afterend', glance);
    }
    return glance;
}

function applyOpsSignalAttentionGlanceState() {
    const glance = ensureOpsSignalAttentionGlance();
    if (!glance) return;
    const text = document.getElementById('ops-signal-attention-glance-text');
    if (agentOpsCollapsed) {
        glance.hidden = true;
        glance.classList.remove('has-warn', 'has-bad', 'has-not-set', 'has-partial', 'has-not-wired');
        return;
    }
    const parsed = parseOpsSignalHealth(opsSignalHealthCache);
    if (parsed.wash === 'ok' || !parsed.glanceLine) {
        glance.hidden = true;
        glance.classList.remove('has-warn', 'has-bad', 'has-not-set', 'has-partial', 'has-not-wired');
        return;
    ...
    const isPartial = parsed.glanceLine.includes('Partial');
    const isNotWired = parsed.glanceLine.includes('Not wired');
    glance.hidden = false;
    glance.classList.toggle('has-not-wired', isNotWired);
    glance.classList.toggle('has-not-set', parsed.st === 'notconfigured' && !isNotWired);
    glance.classList.toggle('has-partial', isPartial);
    glance.classList.toggle('has-warn', parsed.wash === 'warn' && parsed.st !== 'notconfigured' && !isPartial && !isNotWired);
    glance.classList.toggle('has-bad', parsed.wash === 'bad');
    if (text) text.textContent = parsed.glanceLine;
    glance.setAttribute('role', 'button');
    glance.tabIndex = 0;
    if (isNotWired || parsed.st === 'notconfigured') {
        glance.title = 'Signal alerts are not wired yet — click to open Settings';
        glance.setAttribute(
            'aria-label',
            'Signal not wired — click to open Settings Credentials'
        );
    } else if (isPartial) {
        glance.title = 'Signal REST API is not wired yet — click to open Settings';
        glance.setAttribute(
            'aria-label',
            `${parsed.glanceLine} — click to open Settings Credentials`
        );
    } else {
        glance.title = 'Signal probe failed — click to check Settings';
        glance.setAttribute(
            'aria-label',
            `${parsed.glanceLine} — click to open Settings Credentials`
        );
...
function activateOpsSignalAttentionGlance() {
    if (agentOpsCollapsed) applyOpsCollapsed(false);
    document.getElementById('settings-btn')?.click();
    requestAnimationFrame(() => {
        setTimeout(() => {
...
function wireOpsSignalAttentionGlanceClick(glance) {
    if (!glance || glance.dataset.opsSignalAttentionWired === '1') return;
    glance.dataset.opsSignalAttentionWired = '1';
    const activate = () => activateOpsSignalAttentionGlance();
    glance.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        activate();
...
function renderOpsHealth({ version, insights, sched, deliveries, agents, live, redmine, ollama, brave, browser, mcp, cursor, perplexity, mastodon, telegram, slack, signal, sessionFiles }) {
    ...
    if (signal) {
        opsSignalHealthCache = signal;
    } else {
        opsSignalHealthCache = null;
    }
    applyOpsSignalAttentionGlanceState();
...
function renderOverviewSchedules(schedules, deliveries) {
    ...
    schedules.slice(0, 3).forEach((s) => {
        ...
        // Empty task: match schedule preview "None yet" (bare blank after · reads like missing data).
        const taskRaw = String(s.task || '').trim() || 'None yet';
        const task = taskRaw.slice(0, 40);
        btn.innerHTML = `<div><div class="ops-row-title">${escapeHtml(id)}</div><div class="ops-row-meta">next ${escapeHtml(next)} · ${escapeHtml(task)}${taskRaw.length > 40 ? '…' : ''}</div></div>`;
        btn.addEventListener('click', () => {
            body.querySelectorAll('.ops-row.is-selected').forEach((el) => el.classList.remove('is-selected'));
            btn.classList.add('is-selected');
...
function appendOverviewLastDeliveryRow(body, d) {
    ...
    // Empty summary: match delivery preview "None yet" (omit left a gap vs preview).
    const summaryRaw = String(d.summary || '').trim() || 'None yet';
    const summary = summaryRaw.slice(0, 40);
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'ops-row';
    const metaBits = [id, ageLabel, summary];
    btn.innerHTML =
        `<div><div class="ops-row-title">Last delivery</div>` +
        `<div class="ops-row-meta">${escapeHtml(metaBits.join(' · '))}${summaryRaw.length > 40 ? '…' : ''}</div></div>`;
    btn.addEventListener('click', () => {
        body.querySelectorAll('.ops-row.is-selected').forEach((el) => el.classList.remove('is-selected'));
        btn.classList.add('is-selected');
    ...
    if (list) {
        ...
        if (!schedAll.length) {
            ...
            schedFiltered.forEach((s) => {
                ...
                const when = s.cron ? `cron ${s.cron}` : s.at ? `at ${s.at}` : 'None yet';
                const next = s.next_run || s.nextRun || 'None yet';
                // Empty task: match schedule preview "None yet" (bare blank meta reads like missing data).
                const task = String(s.task || '').trim() || 'None yet';
    ...
    if (delList) {
        ...
        if (!delAll.length) {
            ...
            delFiltered.slice(0, 8).forEach((d) => {
                ...
                // Empty summary: match delivery preview "None yet" (bare blank after · reads like missing data).
                const summary = String(d.summary || '').trim() || 'None yet';
                // Empty schedule id: match schedule list "Unknown" (bare "schedule" reads like missing data).
...
async function refreshAgentOps(opts = {}) {
    ...
    try {
        ...
        const signal = (features || []).find((h) =>
            String(h.name || '').toLowerCase() === 'signal'
        );
        renderOpsHealth({
            version,
            insights,

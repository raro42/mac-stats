...
def looks_like_ratchet_results_glance_ask(q: str) -> bool:
    ...
    needles = (
        ...
        "keep p95",
        "p95 keep gap",
        "95th percentile keep gap",
        "p95 gap between keeps",
        "/discard-p95",
        "discard p95",
        "p95 discard gap",
        "/keep-p10",
        "keep p10",
        "p10 keep gap",
        "10th percentile keep gap",
        "p10 gap between keeps",
        "/discard-p10",
        "discard p10",
        "p10 discard gap",
        "/keep-p25",
        "keep p25",
        "p25 keep gap",
        "25th percentile keep gap",
        "p25 gap between keeps",
        "/discard-p25",
        "discard p25",
        "p25 discard gap",
        "/keep-iqr",
        "keep iqr",
        "iqr keep gap",
        "interquartile keep gap",
        "iqr gap between keeps",
        "/discard-iqr",
        "discard iqr",
        "iqr discard gap",
        "/keep-mad",
        "keep mad",
        "mad keep gap",
        "median absolute deviation keep gap",
        "mad gap between keeps",
        "/discard-mad",
        "discard mad",
        "mad discard gap",
        "/keep-histogram",
        ...
        "/discard-histogram",
        "discard histogram",
        "discard gap histogram",
        "/keep-hist",
        "keep hist",
        "/keep-gini",
        "keep gini",
        "gini keep gap",
        "gini coefficient keep gap",
        "gini gap between keeps",
        "/discard-gini",
        "discard gini",
        "gini discard gap",
        "/keep-mode",
        ...
        "keep kurtosis",
        "kurtosis keep gap",
        "excess kurtosis keep gap",
        "kurtosis gap between keeps",
        "/discard-kurtosis",
        "discard kurtosis",
        "kurtosis discard gap",
        "/keep-skew",
        "keep skew",
        "skew keep gap",
        "skewness keep gap",
        ...
        "keep std",
        "std keep gap",
        "standard deviation keep gap",
        "std gap between keeps",
        "/discard-std",
        "discard std",
        "std discard gap",
        "/recent-keeps",
...
def looks_like_vague_followup(q: str) -> bool:
    n = (q or "").strip().lower().rstrip("?!.").strip()
    if len(n) > 96:
        return False
    if any(x in n for x in ("http", "skill:", "cursor_agent:", "redmine", "brave_search", "perplexity")):
        return False
    if " about " in n:
        return False
    investigate = any(
    ...
    ) or n in ("further", "go further", "go deeper")
    tailor = any(
        x in n
        for x in (
            "tailor it to my interest",
            "tailor it to my interests",
            "tailor to my interest",
            "make it more relevant to me",
        )
    ) or n in ("tailor it", "make it relevant")
